...
## 지역별 / 연령 범주별 / (해당 인구 수/지역 인구 수*100) 값
```{python}
...
## address_new에서 시/도 값 분리해서 'region' 변수 만들기
```{python}
first_tokens=[]
...
## 노인 인구가 많으면 시장 갯수가 많이 형성되었을 것이다.

```{python}
import numpy as np
market_pop['old_up'] = np.where(market_pop['age_over_60'] >= market_pop['age_over_60'].mean(), 1 ,0)
market_pop['old_down'] = np.where(market_pop['age_over_60'] < market_pop['age_over_60'].mean(), 1 ,0)
market_pop.head()
```


```{python}
market_pop.groupby('level',as_index=False).agg(market_up_count = ('old_up', 'sum')
                               , market_down_count = ('old_down','sum'))
```
```{python}
market_pop.groupby('market_scale',as_index=False).agg(market_up_count = ('old_up', 'sum')
                               , market_down_count = ('old_down','sum'))
```
...
편의시설이 YY 인데 인구 많은데야? 인구 많으면 편의시설 좋아?
