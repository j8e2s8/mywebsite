...
merge1['region'] == '서울특별시  '
```
region 컬럼의 값이 '**  ' 공백이 포함되어 있음을 알 수 있다.
이 상태에서 merge하면 merge가 안됨.
...
subset2['region'] = subset2['region'].strip()
subset2
```
...
```{python}
merge1 = pd.read_excel('subset2.xlsx')
merge1.head()
```
...
```{python}
market_pop = pd.merge(market2, subset2, how='inner', on='region')
...
```{python}
market_pop.query('level == "high"')['region']
```
