...
plt.clf()
sns.histplot(train_df[train_df['MSZoning'] == 'RH']['SalePrice'], stat='density')
sns.histplot(train_df[train_df['MSZoning'] == 'FV']['SalePrice'], stat='density')
sns.histplot(train_df[train_df['MSZoning'] == 'C (all)']['SalePrice'], stat='density')
sns.histplot(train_df[train_df['MSZoning'] == 'RL']['SalePrice'], stat='density')
sns.histplot(train_df[train_df['MSZoning'] == 'RM']['SalePrice'], stat='density')
plt.title('MSZoning 범주별, SalePrice 분포')
plt.tight_layout()
plt.rcParams['axes.unicode_minus'] = False
plt.rcParams['font.family'] = 'Malgun Gothic'
plt.show()
...
LotFrontage의 분포를 보면 이상치 있어보임.
LotFrontage과 SalePrice의 관계를 보면 많이 퍼져있기는한데 이상치를 제외하면 양의 관계가 있는 것 같기도 함.
...
LotArea의 분포를 보면 이상치 있어보임.
LotArea과 SalePrice의 관계를 보면 일부의 집 때문에 굉장히 퍼지는 관계임.
...
plt.clf()
sns.histplot(train_df[train_df['Street']=='Pave']['SalePrice'], stat='density')
sns.histplot(train_df[train_df['Street']=='Grvl']['SalePrice'], stat='density')
plt.title('MSZoning 범주별, SalePrice 분포')
plt.tight_layout()
plt.show()
...
plt.clf()
sns.histplot(train_df[train_df['Alley']=='Pave']['SalePrice'], stat='density')
sns.histplot(train_df[train_df['Alley']=='Grvl']['SalePrice'], stat='density')
plt.title('MSZoning 범주별, SalePrice 분포')
plt.tight_layout()
plt.show()
...
범주별 분포가 많이 다름. <- 범주별로 집 가격 차이가 있을 것임.
...
plt.clf()
sns.histplot(train_df[train_df['LotShape']=='Reg']['SalePrice'], stat='density')
sns.histplot(train_df[train_df['LotShape']=='IR1']['SalePrice'], stat='density')
sns.histplot(train_df[train_df['LotShape']=='IR2']['SalePrice'], stat='density')
sns.histplot(train_df[train_df['LotShape']=='IR3']['SalePrice'], stat='density')
plt.title('LotShape 범주별, SalePrice 분포')
plt.tight_layout()
plt.show()
...
plt.clf()
sns.histplot(train_df[train_df['LandContour']=='Lvl']['SalePrice'], stat='density')
sns.histplot(train_df[train_df['LandContour']=='Bnk']['SalePrice'], stat='density')
sns.histplot(train_df[train_df['LandContour']=='HLS']['SalePrice'], stat='density')
sns.histplot(train_df[train_df['LandContour']=='Low']['SalePrice'], stat='density')
plt.title('LotShape 범주별, SalePrice 분포')
plt.tight_layout()
plt.show()
...
전기, 가스만 사용가능한 집이 1개만 있고, 그 외에는 모두 사용 가능한 집임.
이상치 존재해 보임.
...
plt.clf()
sns.histplot(train_df[train_df['LotConfig']=='Inside']['SalePrice'], stat='density')
sns.histplot(train_df[train_df['LotConfig']=='Corner']['SalePrice'], stat='density')
sns.histplot(train_df[train_df['LotConfig']=='CulDSac']['SalePrice'], stat='density')
sns.histplot(train_df[train_df['LotConfig']=='FR2']['SalePrice'], stat='density')
sns.histplot(train_df[train_df['LotConfig']=='FR3']['SalePrice'], stat='density')
plt.title('LotConfig 범주별, SalePrice 분포')
plt.show()
...
plt.clf()
sns.histplot(train_df[train_df['LandSlope']=='Gtl']['SalePrice'], stat='density')
sns.histplot(train_df[train_df['LandSlope']=='Mod']['SalePrice'], stat='density')
sns.histplot(train_df[train_df['LandSlope']=='Sev']['SalePrice'], stat='density')
plt.title('LandSlope 범주별, SalePrice 분포')
plt.show()
...
경사가 심하면 건축비용이 많이 든다길래 집값에 차이가 있을 줄 알았는데, 범주별로 분포의 차이가 없어보임. 
파란색 분포에 이상치가 있어보임.
...
```
       ...
       Timber	Timberland
       Veenker	Veenker
분포 차이가 있는 지역이 있는 것 같음. <- 지역마다 집값의 차이가 있다고 볼 수 있음.
집값이 비슷한 지역끼리 묶어서 분석해볼 수도 있을 것 같음.
빨간색, 파란색 분포에서 이상치가 있어보임.
보라색, 하늘색, 주황색, 회색, 분홍색 지역은 특정 집값에 몰려있는 것 같음.
집 데이터가 가장 많은 지역은 NridgHt임.
그 다음이 CollgCr
